...
public class LotteryDevice extends PApplet {
...
public void draw() {
  ...
  //draw 
  gps_coordinate(12.234234234f, 43.23232f);
  
  draw_picture();
  draw_compass(TWO_PI*random(1.0f));
  draw_temp_and_humi(100.00f, 90.32f);
  draw_satellitePosture(rotate_x, rotate_y, rotate_z);
  
  rotate_y += 0.02f;
  rotate_x += 0.05f;
  rotate_z += 0.02f;
  compass_value += HALF_PI*0.01f;
  ...
  if (system_state == 1) {
    while ( myPort.available () > 0 ) {
      if (is_first_change_to_system_state_1 == 0) {       // Arduino will send the length of img at first, get it
        ...
        pic_length = PApplet.parseInt(inString);
 ...
 */
public boolean Get_Valid_Serial_Number() {
  int waitingCount = 0;
  while (myPort.available() <= 0) {
    delay(1);
    if (waitingCount++ > 100) {
...
public boolean mouseIn(int x1, int y1, int x2, int y2) {
...
public void draw_satellitePosture(float rotate_x, float rotate_y, float rotate_z) {
  pushMatrix();
...
public void draw_compass(float degree) {
  pushMatrix();
...
public void draw_temp_and_humi(float temp, float humi) {
...
public void gps_coordinate(float longitude, float latitude) {
...
class Lottery{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--hide-stop", "LotteryDevice" };
    if (passedArgs != null) {
